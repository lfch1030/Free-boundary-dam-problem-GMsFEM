function [x, error, iter, flag, lambdamax, condnumber] = Padcg(x, b,br, max_it, tol,dom,dom_ov,Nx,Ny)
...
%                           1 = no convergence given max_it
  ...
  Ax=applay_Aadd_inv(x,dom,dom_ov,Nx,Ny);
%  r=0*b;
  r = b -Ax;
...
%      paux=applay_A(p,dom,Nx,Ny);
     Ap=applay_Aadd_inv(p,dom,dom_ov,Nx,Ny);
%     q=0*p;
     q=Ap;
     alpha(iter) = rho / (p'*q );
