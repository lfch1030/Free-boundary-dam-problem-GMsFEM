function [x, error, iter, flag, lambdamax, condnumber] = Apcg_ADD_2LtwoCMSr(x, b,br, max_it, tol,dom,dom_ov,Nx,Ny,mu,A0)
...
%  error = norm( r ) / bnrm2;  
  ...
  fprintf(' PCG2L2CR residual(%d) = %g\n',iter,error)
  if ( error < tol ) return, end

  for iter = 1:max_it                       % begin iteration

      zlocal  = apply_A2ladd_inv(r,dom_ov,Nx,Ny,A0);
      z0=apply_P0BADMSr(r,dom_ov,Nx,Ny,A0);
% % %       subplot(2,2,1)
...
% pause(0.2)
     ...
     fprintf('[ %f, %f ] PCG2L2CR residual(%d) = %g\n',norm(r),bnrm2,iter,error)
