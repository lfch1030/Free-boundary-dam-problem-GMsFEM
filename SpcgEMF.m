function [x, error, iter, flag, lambdamax, condnumber] = SpcgEMF(x, b,br, max_it, tol,dom,dom_ov,A0,Nx,Ny)
...
G=Gamma_index(dom,Nx,Ny);
  flag = 0;                                 % initialization
  iter = 0;
  bnrm2 = norm( br );
  if  ( bnrm2 == 0.0 ), bnrm2 = 1.0; end
  Ax=applay_S(x,dom,Nx,Ny);
%  r=0*b;
  r = b -Ax;
  r(G)=0;
...
%  error = norm( r ) / bnrm2;  
  ...
  for iter = 1:max_it                       % begin iteration
      ...
      z  = applay_sumStildeIinvEMF(r,dom,dom_ov,A0,Nx,Ny);
%      z  = applay_sumSIinvEMF(r,dom,dom_ov,A0,Nx,Ny);
%      z  = pc(r,iter);
...
%     q=0*p;
     ...
     fprintf(' PCG SchurEMF residual(%d) = %g\n',iter,error)
