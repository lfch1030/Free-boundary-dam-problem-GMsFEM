function dom_ov=localeigenvectors2(x,dom_ov, Nx,Ny,add)

for i1=1:Nx+1
    for i2=1:Ny+1
        Ifree=dom_ov(i1,i2).free;
        A=dom_ov(i1,i2).A(Ifree,Ifree);
        M=dom_ov(i1,i2).Mass(Ifree,Ifree);
        [Q,D]=eig(full(A),full(M));
%        [Q,D]=eig(full(A));
        [lambda,I]=sort(diag(D),'ascend');
        Q=Q(I,I);
        Ig=dom_ov(i1,i2).Igfree;
