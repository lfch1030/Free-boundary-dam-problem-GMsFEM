...
[dom,dom_ov,b,xd, A0,A0MS, A0EMF,A02C,A02CMS,A02CMS2,A02Cr,iterlambdaEMF,A02Ctilde]=maintwoDB(ax,bx,ay,by,Nx,Ny,nx,ny,mu,k1perm,max_it,tol,ov);
...
%[x2Ctilde, error2Ctilde, iter2Ctilde, flag2Ctilde, lambdamax2Ctilde, condnumber2Ctilde] = Apcg_ADD_2LtwoCtilde(b*0, b,b, max_it, tol,dom,dom_ov,Nx,Ny,mu,A02Ctilde);
...
%iterlambdaEMF
%Siter(exponent,:)=[mu+1, iter1L, iter2L,iterMS,  iterEM,iter2C,iter2CMS,iter2CMS2,iterR]
Siter(exponent,:)=[mu+1, iter2L,iterMS,  iterEM,iter2C]
% % % % % Siter(exponent,:)=[mu+1,iter2L, iterMS,  iterEM,iter2C,iter2Ctilde]
% % % % % % Siter(n,N)=iter2L;
% % % % % 
...
% % % % % %Scond(exponent,:)=[condnumber1L,condnumber2L,condnumberMS,condnumberEM,condnumber2C, condnumber2CMS,condnumber2CMS2,condnumberR ];
 Scond(exponent,:)=[condnumber2L, condnumberMS,condnumberEM,condnumber2C];
...
% % % % % badeig;
% % % % % badeigtilde;
% % % % % Sbad(exponent).bad=bad;
% % % % % Sbad(exponent).badtilde=badtilde;
% % % % % 
% % % % % [badtilde,badtilde(:,2)*0 ,  bad]
% % % % % 
...
matrix_BmgA
lambda=sort(eig(full(B)));
alllambda(n).l=lambda;
figure(n)
plot(abs(lambda),'*')
 pause(0.1)   
